...
## v19 debug（高 IoU/低 mAP 的再诊断：记录类别混淆与遗漏 GT）
- **新增输出：类别混淆矩阵与未匹配 GT 计数**
  - 每次评估都会在 `debug_eval/eval_summary.json` 写出每类 GT/预测数量、被成功匹配的预测/GT 数以及“预测类→真实类”的混淆矩阵，用来量化“框回归对了但分类错了”的情况。【F:utils/metrics.py†L25-L119】【F:utils/metrics.py†L121-L154】
  - 同时记录未被任何预测覆盖的 GT 数（`per_class_unmatched_gt`），帮助确认哪些类别的召回仍然为 0，从而解释“可视化框数量对不上、mAP 很低但 IoU 还高”的现象。【F:utils/metrics.py†L29-L118】【F:utils/metrics.py†L121-L154】
- **如何解释你最新的评估现象**（mAP≈0.165、mean IoU≈0.74，class2/5 有分，其余类接近 0）：
  1. IoU 计算只基于已匹配的框，因此少量 class2/5 框若定位正确，IoU 会被拉高；但长尾类（class0/1/4）缺少预测或被判成高频类，AP 为 0，拉低 mAP。
  2. 通过 `eval_summary.json` 的混淆矩阵可验证：若大量预测集中在某一列（例如 GT=5），说明模型把不同类别都判成 class5；这会在可视化中表现为红框数量少/类别偏移，即使部分位置重叠。
  3. 查看 `per_class_unmatched_gt` 若长尾类 GT 大多未被匹配，说明召回仍不足，需要降低阈值、提高该类权重或补充数据；若匹配到了但混淆矩阵显示被判成其他类，则主要是分类头/数据分布的问题，需要重加权或重采样。
- **建议动作**：跑一次 `python eval.py --checkpoint <ckpt> --split test --debug-dir debug_eval`，检查 `eval_summary.json`：
  - 若某行（GT 类）几乎全为 0，同时 `per_class_unmatched_gt` 大，先解决召回（阈值/损失权重/训练轮次）。
  - 若某列占多数，说明模型把多类预测成同一类，需强化分类（class weight/focal、数据平衡、增加分类层）并在可视化中针对该类调低阈值观察。

## v20 debug（模型是否过于简单 vs. 数据集问题？）
- **立即打印并写入 `dataset_stats.json` 的分布告警**：训练启动时记录 train/valid/test 各类样本数、缺失类别和最大/最小频次比；若某 split 缺类或 max/min ≥10，会在控制台给出警告并写入日志，提示这会把对应类的 AP 固定为 0 或让分类严重偏向头部类。【F:train.py†L108-L134】【F:train.py†L144-L156】
- **为什么高 IoU 但 mAP 仍低、可视化框数对不上？**
  - 数据分布仍是主因：最新 eval 显示 class2/5 有 AP，而 class0/1/4 接近 0，说明长尾类几乎没被匹配。即便定位 IoU 高，缺失 GT 或极少的样本让这些类的 AP 固定为 0，整体 mAP 被拖低。
  - 模型结构已能产生 0.7+ 的 IoU，说明回归链路基本正常；但分类和召回在长尾类上崩溃，与数据稀缺/失衡更吻合，而非“模型太简单”。
- **结论与建议**
  - 先用 `dataset_stats.json`/控制台告警确认验证、测试是否存在缺失类；若有缺失或极端失衡，不增加数据/重采样/调整权重，再换更复杂模型也难以抬高 mAP。
  - 在现有模型上，可针对缺失/长尾类提升 class weight、使用 focal loss gamma>1、过采样或补标；若数据平衡后仍不足，再考虑更大模型或多尺度训练。

...
from datasets.voc2012 import PascalVOC2012, collate_fn, compute_class_frequencies, gather_label_stats
...
def create_dataloaders():
    train_dataset = PascalVOC2012(split="train", augment=True)
    val_dataset = PascalVOC2012(split="val", augment=False)
...
def _collect_debug_batch(model, decoded, preds, metas) -> Dict[str, Any]:
    ...
    for det, meta, raw in zip(decoded, metas, preds):
        ...
        if low_thresh.numel() and meta["boxes_orig"].numel():
            ...
            gt_xyxy[:, 0::2] *= orig_w
            gt_xyxy[:, 1::2] *= orig_h
            pred_xyxy = PascalVOC2012.unletterbox_boxes(
                low_thresh[:, :4].cpu(),
...
def _save_debug(epoch: int, payload: Dict[str, Any]):
    ...
    out_path = config.LOG_DIR / f"voc_debug_epoch_{epoch:03d}.json"
...
def main(args):
    ...
    freq = compute_class_frequencies(train_dataset.ann_dir)
    label_stats = {
        "train": gather_label_stats(config_voc2012.ANNOTATION_DIRS["train"]),
        "val": gather_label_stats(config_voc2012.ANNOTATION_DIRS["val"]),
    }
    config.LOG_DIR.mkdir(parents=True, exist_ok=True)
    with open(config.LOG_DIR / "voc_dataset_stats.json", "w", encoding="utf-8") as f:
    ...
    for epoch in range(1, config.EPOCHS + 1):
        ...
        if map50 > best_map:
            best_map = map50
            ckpt_path = config.CHECKPOINT_DIR / "voc_best.pt"
            torch.save(
                {
    ...
    ckpt_last = config.CHECKPOINT_DIR / "voc_last.pt"
    torch.save(
        {
            "model_state": model.state_dict(),
            "optimizer_state": optimizer.state_dict(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=config_voc2012.EPOCHS, help="Override epochs")
    args_parsed = parser.parse_args()
    if args_parsed.epochs != config_voc2012.EPOCHS:
        config_voc2012.EPOCHS = args_parsed.epochs
    main(args_parsed)
